...
function mountains(mesh, n, r) {
    ...
    for (var i = 0; i < mesh.vxs.length; i++) {
        ...
        for (var j = 0; j < n; j++) {
            var m = mounts[j];
            newvals[i] += Math.pow(Math.exp(-((p[0] - m[0]) * (p[0] - m[0]) + (p[1] - m[1]) * (p[1] - m[1])) /
                (2 * r * r)), 2);
        }
    }
    return newvals;
...
const PriorityQueue = {};
function getTerritories(render, ...args) {
    var h = render.h;
...
function getBorders(render, ...args) {
...
function mergeSegments(segs) {
    ...
    while (true) {
        ...
        for (var i = 0; i < segs.length; i++) {
            ...
            else if (adj[path[path.length - 1]].length == 2 &&
